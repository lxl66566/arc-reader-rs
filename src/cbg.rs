...
//! decoding, following GARBro's ImageCBG.cs implementation.
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            for p in 0..pixel_size {
                let mut avg: u8 = 0;
                if x > 0 {
                    avg = avg.wrapping_add(output[pixel + p - pixel_size]);
                }
                if y > 0 {
                    avg = avg.wrapping_add(output[pixel + p - stride]);
                }
                if x > 0 && y > 0 {
                    // average of two values
                    avg = output[pixel + p - pixel_size] / 2 + output[pixel + p - stride] / 2;
                    // this matches GARBro's per-byte logic
                }
                if avg != 0 {
                    output[pixel + p] = output[pixel + p].wrapping_add(avg);
                }
            }
        }
    }
...
fn decrypt_v2(
    ...
    for i in 0..y_blocks {
        ...
        if block_offset < remaining_data.len() {
            let block_data = &remaining_data
                [block_offset..block_offset + block_len.min(remaining_data.len() - block_offset)];
            decode_block(
                block_data,
                &tree1,
                &tree2,
        ...
        dst += w_align * 32; // 8 rows * width * 4 bytes
    }

    // --- Step 6: Decode alpha channel if 32bpp ---
    ...
    // --- Step 7: Crop to actual dimensions and convert to RGBA ---
...
fn decode_block(
    ...
    // Read block_size (variable-length integer)
    ...
    let total_shorts = block_count * 64 * 3; // 3 channels (Y, Cb, Cr)
    let mut color_data = vec![0i16; total_shorts.max(block_size)];

    // --- DC coefficients (Tree1) ---
    // GARBro guards: `input.Position < input.Length` in the loop condition.
    let mut acc: i32 = 0;
    let mut i = 0;
    ...
    // GARBro guards: `input.Position < input.Length` in both outer and inner loops.
    i = 0;
    while i < block_size && !bits.is_eof() {
    ...
    for blk in 0..block_count {
        ...
        for j in 0..64 {
            ...
            if dst_start + p + 3 < output.len() {
                output[dst_start + p] = float_to_byte(b);
                output[dst_start + p + 1] = float_to_byte(g);
                output[dst_start + p + 2] = float_to_byte(r);
                // Alpha set later by decode_alpha, default 0xFF is handled in
                // crop_and_convert
            }
        }

        dst_start_checked_add(output.len(), dst_start, 32); // advance by 8
                                                            // pixels * 4 bytes
    }
}
...
#[allow(clippy::unnecessary_wraps, clippy::match_same_arms)]
fn dst_start_checked_add(_len: usize, _start: usize, _inc: usize) -> usize {
    0
}
...
/// DCT scaling table (8x8), from GARBro's ParallelCbgDecoder.
